...
import { groupRecords } from "../../services/groups";
import { deleteDevice, devicesRecords } from "../../services/devices";
...
class ManageCities extends Component {
  ...
  delete = () => {
    ...
    deleteDevice(this.state.selectedId)
      .then((res) => {
        this.getData();
      })
      .catch((error) => {
  ...
  toggleDeviceHistoryModal = (id) => {
    this.setState({
      selectedId: id,
      showDeviceHistoryModal: !this.state.showDeviceHistoryModal,
    });
  };
    ...
    return (
      <Fragment>
                      ...
                      <thead>
                        <tr>
                          ...
                          <th className="relative py-3 border-b border-gray-200 bg-gray-50 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            {this.renderOrder("name", "right-0")}
                            Name
                          </th>

                          <th className="relative  py-3 border-b border-gray-200 bg-gray-50  text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            {this.renderOrder("description", "right-3")}
                            Description
                          </th>
                          <th className="relative  py-3 border-b border-gray-200 bg-gray-50  text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            {this.renderOrder("code", "right-0")}
                            Code
                          </th>
                          <th className="relative py-3 border-b border-gray-200 bg-gray-50  text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            {this.renderOrder("hardware", "right-2")}
                            Hardware
                          </th>
                          <th className="relative  py-3 border-b border-gray-200 bg-gray-50 text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            {this.renderOrder("status", "right-0")}
                            status
                          </th>
                          <th className="relative pr-16 py-3 border-b border-gray-200 bg-gray-50 text-right text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                            Actions
                          </th>
                        </tr>
                      </thead>
                          ...
                          >
                            ...
                            <td className="px-4 py-2 whitespace-no-wrap text-right border-b border-gray-200 text-xs leading-5 font-medium">
                              ...
                              {this.context.state.delete ? (
                                <button
                                  onClick={() => {
                                    this.toggleDeviceHistoryModal(item.id);
                                  }}
                                  className="cursor-pointer text-blue-600  hover:text-blue-900"
                                >
                                  History
        ...
        {this.state.showCreate || this.state.showUpdate ? (
          <DeviceModel
            title={this.state.showUpdate ? "Update Device" : "Add Device"}
        ...
        {this.state.showDeviceHistoryModal ? (
          <DeviceHistory
            title="Device Location History"
            close={this.toggleDeviceHistoryModal}
            action={this.getHistory}
            id={this.state.selectedId}
            closeParent={this.props.close}
          ></DeviceHistory>
        ) : null}
      </Fragment>
    );
...
ManageCities.contextType = AppContext;
