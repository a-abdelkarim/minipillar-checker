...
import { eventSite, eventItem, eventDelete } from "../../services/events";
import { siteRecord } from "../../services/sites";
import { itemRecord } from "../../services/items";
...
class Events extends Component {
  ...
  refresh = () => {
    if (this.props.eventType === "site") {
      eventSite(this.props.eventId)
        .then((res) => {
          this.setState({
            items: res.items,
          });
    ...
    } else if (this.props.eventType === "item") {
      eventItem(this.props.eventId)
        .then((res) => {
          this.setState({
            items: res.items,
          });
  ...
  render() {
    return (
      <Fragment>
        ...
        <div className="arabic fixed h-full w-full  pr-20 right-0 bottom-0 z-30 bg-black bg-opacity-50  ">
            ...
            <div className=" relative w-full flex justify-between items-center flex-wrap sm:flex-no-wrap">
              <div className="w-full">
                <h3 className="text-lg leading-6 font-bold text-gray-900">
                  الاحداث [ {this.state.item.name} ]
                </h3>
                {this.state.user.type != "viewer" ? (
                  <p className="mt-1 text-sm leading-5 text-gray-700">
                    عرض كل الاحدات المتعلقة بالموقع{" "}
                    <button
                      onClick={() => {
                        this.toggleModal(0);
                      }}
                      className="mr-10 mt-3 top-0 px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue active:bg-blue-700 transition ease-in-out duration-150"
                    >
                      إضافه حدث{" "}
                    </button>
                    <button
                      onClick={() => {
                        this.toggleOtherEvent(0);
                      }}
                      className="mr-10 mt-3 top-0 px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue active:bg-blue-700 transition ease-in-out duration-150"
                    >
                      جرد
              ...
              <button
                onClick={() => {
                  this.props.hide(null, 0);
                }}
                type="button"
                className=" absolute p-2 z-30  ml-5  left-0 border border-gray-300 rounded-full text-sm leading-5 font-medium text-gray-700 hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-50 active:text-gray-800 transition duration-150 ease-in-out"
              >
                <svg
                  id="Outlined"
        ...
        {this.state.showOtherEventModal ? (
          <ExtraEvent
            title="تسجيل جرد"
            id={this.state.selectedId}
            eventId={this.props.eventId}
